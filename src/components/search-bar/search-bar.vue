    ...
    <div class="um-search-input">
      <div class="um-search-synthetic-ph" ref="syntheticPhRef">
        <span class="um-search-synthetic-ph-container" ref="container">
          ...
          <span
            class="um-search-synthetic-ph-placeholder"
  ...
  computed: {
    cls() {
      return {
        [`${prefixCls}`]: true,
        [`${prefixCls}-start`]:
          this.focus || (this.curValue && this.curValue.length > 0)
      }
    },
    clearCls() {
      return {
        ...
        [`${prefixCls}-clear-show`]:
          this.focus || (this.value && this.value.length > 0)
      }
    },
    cancelCls() {
      return {
      ...
      if (this.$refs.search.className.indexOf("um-search-start") > -1) {
        ...
        if (!this.showCancelButton) {
          this.$refs.rightBtnRef.style.marginRight = `-${
            this.$refs.rightBtnRef.offsetWidth
          }px`
        }
      }
    }
